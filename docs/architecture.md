...
Future one-line bootstrap: [install.sh](../install.sh) installs apt-based OS prerequisites, bootstraps the app, renders templates, validates installs, and can install service files with `--install-services`. llama.cpp/models/network still manual. Spec: [installer-bootstrap-contract.md](deployment/installer-bootstrap-contract.md).
