...
The default LLM backend is `mock` (`ARK_LLM_BACKEND=mock`). It validates retrieval, prompt assembly, and client wiring without network calls, llama.cpp, or model files. The `openai-compatible` backend is intended for future llama.cpp server use on ark-llm and is opt-in only. **LLM diagnostics** expose passive status (`GET /api/llm/status`, `ark llm status`) without contacting ark-llm, and an explicit active test (`POST /api/llm/test`, `ark llm test`) that sends a tiny diagnostic prompt through the configured client when you choose to verify connectivity. **Local appliance init** (`POST /api/init`, `ark init`) explicitly creates workspace and source directories, optionally seeds an empty catalog and sample text source, then runs passive preflight. **Appliance preflight** (`GET /api/preflight`, `ark preflight`) runs a passive operator checklist for workspace paths, catalog health, source ingest readiness, index backends, import limits, and LLM configuration without hidden network probes or directory creation.
